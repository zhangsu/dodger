...
class SceneNode {
  public:
    ...
    // Tests collision with another node, which always returns false as a pure
    // node is not a physical object, thus collide with nothing. Derived types
    // who do not override this function also inherits this fact.
    virtual bool collideWith(const SceneNode*) const;
    // Tests collision with anything.
    virtual bool colliding() const;
  ...
  private:
    // Tests collision with all descendents of start_node.
    bool colliding(const SceneNode* start_node) const;
